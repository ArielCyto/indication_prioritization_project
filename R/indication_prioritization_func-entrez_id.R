...
get_statistics <- function(data_table, repeats = repeats) {
  ...
  bo_ci <- t(sapply(bo, function(x) {
    ...
    # bo_ci keeps 3 fields per type_sub_population: t0, low_ci and high_ci
...
rank_groups_by_genes <- function(gene_list, repeats = 5, rank_by = 'CI_low'){ # need to add option for specific sub_population/ indication
  ...
  if (submit == TRUE){
    ...
    ## BOOTSTRAPING - calculate the CI per type_sub_population per collection (based on the effector cells average)
    set.seed(42)
