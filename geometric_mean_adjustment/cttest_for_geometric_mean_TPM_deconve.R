...
# histograms
hist(GZMA_PRF1_scaling)
# check myself
unique(annotation_table$group)
group_a_data <- manipulated_deconve_data[,colnames(manipulated_deconve_data) %in% annotation_table$sample_id[annotation_table$group == group_a_name]]


group_a_name <- "lung cancer - LUSC_epithelial"
hist(GZMA_PRF1_scaling[which(colnames(expression_matrix) %in%  annotation_table$sample_id[annotation_table$group == group_a_name])],
     main = paste0(group_a_name, " samples geometric mean vector"))
